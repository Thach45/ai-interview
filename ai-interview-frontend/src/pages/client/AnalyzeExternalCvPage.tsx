...
import { useCvs, useCv } from '../../features/cvs/hooks/useCvs';
...
const AnalyzeExternalCvPage: React.FC = () => {
  ...
  const { cvs, isLoading: isLoadingCvs } = useCvs();
  ...
  return (
    <MainLayout hideSearch={true} maxWidth="1600px" className="px-6 lg:px-10 pt-3 pb-12 flex flex-col">
      <div className="animate-in fade-in duration-500 flex flex-col h-full w-full">
        
        {/* Header Section */}
        <PageHeader 
          title="Phân tích CV theo JD"
          description="Đối chiếu CV của bạn với mô tả công việc để nhận đánh giá chi tiết và gợi ý tối ưu."
        />
              ...
              <div className="mb-5 shrink-0">
                ...
                {isLoadingCvs ? (
                  <div className="flex items-center gap-2 p-3 border border-gray-100 rounded-xl bg-gray-50 text-gray-400 text-xs font-medium">
                    <Loader2 className="animate-spin text-primary" size={14} /> Đang tải danh sách CV...
                  </div>
                ) : (
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => setSearchParams(prev => {
                        ...
                        newParams.set('cvId', e.target.value);
                        return newParams;
                      })}
                      className="w-full appearance-none p-3 pr-10 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium text-gray-800 cursor-pointer shadow-sm hover:border-gray-300"
                    >
                      <option value="" disabled>-- Chọn một CV của bạn --</option>
                      {cvs.map((cv: any) => (
                ...
                {/* Lỗi khi load CV cụ thể */}
                {isCvError && cvId && !isLoadingCvs && (
                  <div className="mt-3 p-3 border border-red-100 rounded-xl bg-red-50 text-red-700 text-xs font-medium flex gap-2 items-start">
                    <AlertCircle className="shrink-0 mt-0.5" size={14} />
                    <span>{(cvError as any)?.response?.data?.message || 'Không thể tải thông tin CV này. Vui lòng thử lại hoặc chọn CV khác.'}</span>
                  </div>
                )}
              </div>
                ...
                {!cvId ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 bg-white overflow-y-auto">
                    ...
                    <p className="text-[13px] text-gray-400 max-w-xs mx-auto mb-4">
                      Vui lòng chọn một CV từ danh sách bên trái để phân tích.
                    </p>
                  </div>
                ) : isLoadingCv ? (
