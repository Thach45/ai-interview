  ...
  return (
    <div className="bg-white border border-border-hairline rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="size-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center shadow-sm">
            <span className="material-symbols-outlined text-[20px]">
              {pkg.icon || "rocket_launch"}
            </span>
          </div>
          <div className="flex gap-2">
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${pkg.isActive ? "bg-green-50 text-green-600 border border-green-100" : "bg-red-50 text-red-600 border border-red-100"}`}
            >
        ...
        {pkg.tagline && (
          <p className="text-[12px] text-text-tertiary mb-4">{pkg.tagline}</p>
        )}
        ...
        <div className="space-y-2 mb-6">
          ...
          {(pkg.features || []).length > 3 && (
            <div className="text-[11px] text-text-tertiary font-medium">
              ... và {(pkg.features || []).length - 3} tính năng khác
            </div>
          )}
        </div>
      </div>
