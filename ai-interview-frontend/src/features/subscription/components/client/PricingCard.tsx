...
export const PricingCard: React.FC<Props> = ({ pkg, index, onBuy }) => {
  ...
  const discount = pkg.oldPrice
    ...
    : 0;
  const creditsLabel =
    pkg.credits === -1 ? "Không giới hạn lượt dùng" : `${pkg.credits} Credit`;
  ...
  return (
    ...
    >
      ...
      {/* Gradient top bar */}
      <div
        className={`h-1.5 w-full rounded-t-2xl bg-gradient-to-r ${gradient}`}
      />
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div
            className={`size-10 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-sm`}
          >
        ...
        <div className="mb-1">
          {(pkg.oldPrice || 0) > 0 && (
            <span className="text-[12px] text-text-tertiary line-through mr-2">
              {formatPrice(pkg.oldPrice || 0)}đ
            </span>
          )}
          {discount > 0 && (
        ...
        <ul className="space-y-2.5 mb-6 flex-1">
          {pkg.features.map((f, i) => (
            ...
            >
              <span className="material-symbols-outlined text-[16px] text-green-500 mt-0.5 shrink-0">
                check_circle
              </span>
              {f}
            </li>
