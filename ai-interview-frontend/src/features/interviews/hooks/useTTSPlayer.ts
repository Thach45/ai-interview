...
export const useTTSPlayer = (sessionId: string, rawStreamText: string) => {
  ...
  const playNextInQueue = () => {
    ...
    // Tìm file Audio đầu tiên đang 'ready'
    const nextItemIndex = queueRef.current.findIndex(item => item.status === 'ready');
    if (nextItemIndex === -1) {
      // Nếu hàng đợi rỗng hoặc đang loading, tắt cờ speaking
      setIsSpeaking(false);
      return;
    }
    ...
    setSpokenText(prev => prev + (prev.length > 0 && !prev.endsWith(' ') ? ' ' : '') + nextItem.text);
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      const sentence = rawStreamText.substring(lastProcessedIndex.current, lastProcessedIndex.current + splitIndex);
