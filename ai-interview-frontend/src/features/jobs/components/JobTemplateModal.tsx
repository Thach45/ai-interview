...
} from "../validations/jobTemplate.validation";
...
export const JobTemplateModal: React.FC<JobTemplateModalProps> = ({
  isOpen,
  onClose,
  template,
}) => {
  const [activeTab, setActiveTab] = useState<"basic" | "content" | "ai">(
    "basic",
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm">
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-border-hairline flex justify-between items-center bg-bg-surface-soft">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-text-tertiary uppercase font-bold tracking-[0.5px]">
                Thư viện JD chuẩn hóa
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="size-8 flex items-center justify-center rounded-full hover:bg-bg-surface text-text-tertiary transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          {activeTab === "basic" && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-[11px] font-bold text-text-tertiary uppercase mb-2">
                    Vị trí tuyển dụng
                  </label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  >
                    <span
                      className={`text-[14px] ${watch("location") ? "text-text-primary font-medium" : "text-text-tertiary"}`}
                    >
                ...
                <div>
                  ...
                  >
                    <span
                      className={`text-[14px] ${watch("employmentType") ? "text-text-primary font-medium" : "text-text-tertiary"}`}
                    >
              ...
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  >
                    <span
                      className={`text-[14px] ${watch("experienceLevel") ? "text-text-primary font-medium" : "text-text-tertiary"}`}
                    >
                ...
                <div className="flex items-center gap-6 pt-6 col-span-2">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                      {...register("isRemote")}
                      className="size-4 rounded border-border-hairline text-primary"
                    />
                    <span className="text-[13px] font-semibold text-text-secondary group-hover:text-text-primary transition-all">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                      {...register("isHotJob")}
                      className="size-4 rounded border-border-hairline text-orange-500"
                    />
                    <span className="text-[13px] font-semibold text-text-secondary group-hover:text-orange-500 transition-all">
                      Vị trí nổi bật
                    </span>
                  </label>
                </div>
              ...
              <div>
                ...
                >
                  {watch("categoryId") ? (
                    getCategoryNames(watch("categoryId") || "").map(
                      (name, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-primary/10 text-primary text-[11px] font-bold rounded-md border border-primary/20"
                        >
                    ...
                    <div className="flex items-center gap-2 text-text-tertiary">
                      ...
                      <span className="text-[13px]">
                        Chọn ngành nghề cho vị trí này...
                      </span>
                    </div>
                  )}
                  <span className="material-symbols-outlined text-text-tertiary ml-auto">
                    expand_more
                  </span>
                </div>
                {errors.categoryId && (
          ...
          {activeTab === "content" && (
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-[11px] font-bold text-text-tertiary uppercase mb-2">
                  Yêu cầu kỹ năng & Kinh nghiệm (Requirements)
                </label>
                <textarea
                  rows={4}
            ...
            <div className="space-y-6">
              <div className="bg-primary/5 border border-primary/10 p-4 rounded-xl flex gap-3">
                ...
                <p className="text-[13px] text-primary/80 leading-relaxed font-medium">
                  Trường <strong>tri thức phỏng vấn</strong> này sẽ được AI sử
                  dụng làm tri thức nền để đặt câu hỏi. Hãy cung cấp bối cảnh
                  chuyên sâu nhất để tối ưu hóa phỏng vấn.
                </p>
              </div>
              <textarea
                rows={12}
              ...
              ></textarea>
              {errors.aiExtractedContext && (
                <p className="text-red-500 text-[11px] mt-1">
                  {errors.aiExtractedContext.message}
                </p>
              )}
            </div>
          )}
        ...
        <div className="px-8 py-5 border-t border-border-hairline bg-bg-surface-soft flex justify-end gap-4">
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2.5 text-[13px] font-bold text-text-secondary hover:text-text-primary transition-colors"
          >
            Hủy bỏ
          </button>
          <button
      ...
      <JobCategoryModal
        ...
        initialSelectedIds={watch("categoryId") ? [watch("categoryId")!] : []}
        multiple={false}
        onApply={(ids) => {
          setValue("categoryId", ids[0] || "");
