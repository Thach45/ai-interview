...
export const LandingHeader: React.FC = () => {
  ...
  return (
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      ...
      >
        {/* Brand */}
        <Link to="/" className="flex items-center gap-2 group shrink-0 pl-3 pr-2">
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          {isAuthenticated ? (
            <Link 
              to="/dashboard" 
              className="group flex items-center gap-1.5 bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-full font-semibold text-[13px] transition-all duration-300 shadow-md shadow-primary/20"
            >
              Vào Dashboard
          ...
          <div className="pt-4 flex flex-col gap-3 border-t border-gray-100 mt-2">
            {isAuthenticated ? (
               <Link 
                 to="/dashboard" 
                 onClick={() => setIsMobileMenuOpen(false)}
                 className="w-full flex justify-center items-center gap-2 bg-primary text-white px-6 py-3 rounded-full font-medium"
               >
                 Vào Dashboard
               </Link>
            ) : (
