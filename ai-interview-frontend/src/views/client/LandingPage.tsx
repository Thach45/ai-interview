...
function ScoreRow({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-1.5 h-1 rounded-full bg-gray-200"><div className="h-full w-[85%] rounded-full bg-primary" /></div>
    </div>
  );
}
        ...
        <section className="border-b border-gray-200 px-6 pb-20 pt-32 sm:pb-28 sm:pt-40">
          <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
            <motion.div {...reveal} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href={isAuthenticated ? '/dashboard' : '/register'} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-primary px-5 py-3 text-sm font-medium text-white shadow-sm transition-colors hover:bg-primary-pressed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">Bắt đầu miễn phí <ArrowRight size={16} aria-hidden="true" /></Link><a href="#features" className="inline-flex min-h-11 items-center justify-center rounded-md border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">Xem tính năng</a></div>
            ...
            <motion.div {...reveal} transition={{ duration: 0.55, delay: prefersReducedMotion ? 0 : 0.12, ease: [0.16, 1, 0.3, 1] }} className="grid min-h-[420px] grid-cols-[1.24fr_0.76fr] gap-3 rounded-xl border border-gray-200 bg-gray-50 p-3 shadow-sm sm:p-4">
              <div className="flex flex-col rounded-lg border border-gray-200 bg-white p-4 sm:p-5"><div className="flex items-center justify-between border-b border-gray-100 pb-4"><div><p className="text-sm font-medium">Arion Interview</p><p className="mt-1 text-xs text-gray-500">Frontend Engineer</p></div><span className="text-xs font-medium text-emerald-700">● Live</span></div><div className="mt-4 flex flex-1 flex-col justify-between rounded-md bg-gray-50 p-4"><div><div className="flex size-7 items-center justify-center rounded-full bg-primary text-[10px] font-medium text-white">AI</div><p className="mt-4 max-w-[31ch] text-sm leading-6 text-gray-800">Hãy kể về một dự án bạn đã cải thiện đáng kể trải nghiệm người dùng.</p></div><div className="rounded-md border border-gray-200 bg-white p-3"><p className="text-xs text-gray-400">Câu trả lời của bạn</p><div className="mt-4 flex items-center justify-between"><span className="text-xs text-gray-500">Đang lắng nghe...</span><span className="flex size-9 items-center justify-center rounded-full bg-primary text-white"><Mic size={15} /></span></div></div></div></div>
              <aside className="rounded-lg border border-gray-200 bg-white p-4 sm:p-5"><p className="text-xs font-medium text-gray-500">Overall score</p><p className="mt-3 text-4xl font-semibold tracking-tight">85<span className="text-sm text-gray-400">/100</span></p><div className="mt-4 h-1.5 rounded-full bg-gray-200"><div className="h-full w-[85%] rounded-full bg-primary" /></div><div className="mt-6 space-y-4"><ScoreRow label="Giao tiếp" value="90" /><ScoreRow label="Kỹ thuật" value="85" /><ScoreRow label="Giải quyết vấn đề" value="80" /><ScoreRow label="Tự tin" value="85" /></div><div className="mt-6 border-t border-gray-100 pt-4"><p className="text-xs font-medium">Feedback</p><p className="mt-2 text-xs leading-5 text-gray-500">Thêm số liệu cụ thể để câu trả lời thuyết phục hơn.</p></div></aside>
            </motion.div>
          </div>
        </section>
