    ...
    <AdminLayout
      ...
      rightAction={
        <button onClick={() => { setEditingPackage(null); setIsFormOpen(true); }}
          ...
          <span className="material-symbols-outlined text-[18px]">add</span>Thêm gói mới
        </button>
      }
    >
      <div className="flex flex-col gap-6">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => <div key={i} className="h-[300px] bg-white border border-gray-100 rounded-2xl animate-pulse" />)}
          </div>
        ) : packages.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {packages.map((pkg) => (
              <PackageCard key={pkg.id} pkg={pkg} onEdit={(p) => { setEditingPackage(p as Package); setIsFormOpen(true); }} onDelete={handleDelete} />
            ))}
          </div>
        )}
      </div>
