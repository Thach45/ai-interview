...
const interceptUpdateProfile = (
  statusCode = 200,
  body = MOCK_UPDATE_SUCCESS,
) => {
  cy.intercept("PUT", "/api/v1/user/me", { statusCode, body }).as(
    "updateProfile",
  );
};

const interceptUpdatePassword = (
  statusCode = 200,
...
const interceptLogout = (statusCode = 200) => {
  cy.intercept("POST", "/api/v1/auth/logout", { statusCode, body: {} }).as(
    "logout",
  );
};
...
const fillPasswordForm = (newPassword: string, confirmPassword: string) => {
  cy.contains("label", "Mật khẩu mới")
    .parent()
    .find('input[type="password"]')
    .clear()
    .type(newPassword);
  cy.contains("label", "Xác nhận mật khẩu")
    .parent()
    .find('input[type="password"]')
    .clear()
...
describe("Profile Page", () => {
  ...
  describe("Form Prefill", () => {
    ...
    it("should prefill dob correctly", () => {
      cy.contains("label", "Ngày sinh")
        .parent()
        .find('input[type="date"]')
        .should("have.value", MOCK_USER.dob);
    });

    it("should prefill bio correctly", () => {
      cy.contains("label", "Giới thiệu")
        .parent()
        .find("textarea")
        .should("have.value", MOCK_USER.bio);
    });
  });
  ...
  describe("Change Password", () => {
    ...
    it("should show error when passwords do not match", () => {
      ...
      cy.contains("form", "Mật khẩu mới").invoke(
        "attr",
        "novalidate",
        "novalidate",
      );
  ...
  describe("Logout", () => {
    ...
    it("should clear localStorage on logout", () => {
      ...
      cy.window().should((win) => {
        ...
        expect(win.localStorage.getItem("auth-storage")).to.include(
