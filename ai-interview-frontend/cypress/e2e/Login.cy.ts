    ...
    describe("Form Validation", () => {
        it("should show error validation styles and messages when fields are empty", () => {
            ...
            cy.get('input[type="email"]')
                ...
                .siblings("p.text-red-500").should("be.visible");
        ...
        it("should show email error for invalid email format", () => {
            ...
            cy.get('button[type="submit"]').click();
            cy.get('input[type="email"]').siblings("p.text-red-500").should("be.visible");
        });

        it("should NOT show email error for valid email format", () => {
            cy.get('input[type="email"]').type(USER.email);
            cy.get('button[type="submit"]').click();
            cy.get('input[type="email"]').siblings("p.text-red-500").should("not.exist");
        });
    });
    ...
    describe("Successful Login", () => {
        it("should send correct credentials and save token correctly", () => {
            ...
            cy.window().should((win) => {
                ...
                expect(authStorage.state.user.fullName).to.equal(MOCK_RESPONSE.data.user.fullName);
            });
        });
        ...
        it("should show loading spinner while awaiting response", () => {
            ...
            cy.get('button[type="submit"]').should("be.disabled").and("contain", "Đang đăng nhập...");
