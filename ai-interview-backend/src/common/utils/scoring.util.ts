export function calculateFinalInterviewResult(
  ...
  const evaluatedQuestions = parsed.questionEvaluations.map((qEval: any) => {
    ...
    if (coreQ && coreQ.criteria) {
      for (const crit of coreQ.criteria) {
        ...
        const match = qEval.criteriaMatches?.find(
          (m: any) => m.criterionId === crit.id,
        );
        if (match) {
  ...
  const avgDomain =
    totalPossibleDomainScore > 0
      ? (totalDomainScore / totalPossibleDomainScore) * 100
      : 0;
  ...
  return {
    generalEvaluation: {
      ...
      domain: {
        ...
        reason:
          'Tính toán tự động dựa trên mức độ đạt tiêu chí chuyên môn (Rubric).',
      },
